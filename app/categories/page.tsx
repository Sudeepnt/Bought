...
const categories = [
  ['ALL', '1,155'], ['BEEF', '184'], ['CHAOS', '28'], ['UNPOPULAR OPINION', '162'],
  ['I WAS WRONG', '141'], ['THE RANT', '118'], ['CONFESSIONS', '96'],
  ['MONEY I SET ON FIRE', '88'], ['THE PITCH THAT GOT REJECTED', '74'],
  ['BUILDING', '69'], ['THE ASK', '61'], ['HIRING', '54'], ['AGENCY ROW', '43'], ['INDIAN D2C', '37'],
] as const;
...
export default function CategoriesPage() {
  ...
  const filteredPlayers = useMemo(() => {
    ...
    return matching.length > 0 ? matching : players.map((player, index) => [
    ...
    >
      <section className="category-page-picker" aria-label="Choose a category">
        {categories.map(([category, bids]) => (
          ...
          >
            <strong>{category}</strong><span>{bids} bids</span>
          </button>
        ))}
      </section>
      ...
      <section className="category-page-layout">
        <div className="route-panel category-ladder-panel">
          <div className="route-panel-head">
            <div>
              ...
              <h2>{activeCategory === 'ALL' ? 'Highest bid across every category.' : `The top spend in ${activeCategory}.`}</h2>
            </div>
            <span className="route-live"><i /> LIVE</span>
          </div>
          <div className="category-page-rows">
            {filteredPlayers.slice(0, 10).map(([name, initials, category, bid, country], index) => (
              <button type="button" className="category-page-row" key={`${name}-${index}`}>
                <span>{String(index + 1).padStart(2, '0')}</span>
                <ProfileAvatar initials={initials} className="category-row-avatar" />
                <strong>{name}<em>{category} / {country}</em></strong>
                <span>{bid}</span>
              </button>
            ))}
          </div>
        ...
        <aside className="category-video-card">
          <div className="category-video-preview"><span>00:30</span><button aria-label="Play the current leader's video" type="button"><Play size={17} /></button></div>
          <span className="eyebrow">CURRENT LEADER / VIDEO TAKE</span>
          <h2>{activeCategory === 'ALL' ? 'Ananya Rao' : filteredPlayers[0][0]}</h2>
          <p>“The position is moving. Watch before you decide to challenge it.”</p>
          <strong>₹18,200 <em>TOP POSITION</em></strong>
        </aside>
      </section>
    </MarketPageShell>
  );
